// script.js
        ...
        let letterDuration = calculateLetterDuration();
        ...
        function playSpecialSymbol(specialSignal) {
            ...
            function processNext() {
                ...
                animationTimer = setTimeout(processNext, letterDuration/3);
            }
            ...
            function processNext() {
                if (index >= animationSequence.length) {
                 ...
                 if (signal ==='[CALL]' || signal ==='[END]'){
                   ...
                   animationTimer = setTimeout(processNext, letterDuration*1.67);                }else{                  if(index>0 && signal === animationSequence[index-1]){                    setSignalPose(' ');                    animationTimer = setTimeout(setSignalPose, letterDuration/3, signal);                    animationTimer = setTimeout(processNext, letterDuration*1.33);                    }else{                    setSignalPose(signal);                    animationTimer = setTimeout(processNext, letterDuration);                      }                                  }                index++;            }
        ...
        speedDecreaseBtn.addEventListener('click', () => {
            speedInput.value = Math.max(parseInt(speedInput.value) - 1, 15);
            validateInt(speedInput, 15, 100, 60);
        });
        
        // 码速加按钮
        speedIncreaseBtn.addEventListener('click', () => {
            speedInput.value = Math.min(parseInt(speedInput.value) + 1, 100);
            validateInt(speedInput, 15, 100, 60);
        });
